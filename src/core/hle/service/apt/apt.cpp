...
void Module::APTInterface::MapProgramIdForDebug(Kernel::HLERequestContext& ctx) {
    // This function got stubbed at some point and no longer does anything on real hardware. It is
    // implemented here so that emulated applications can take advantage of it.
    IPC::RequestParser rp(ctx);
    const auto app_id = rp.PopEnum<AppletId>();
    u64 title_id = rp.Pop<u64>();

    // Real hardware forces the media type to NAND. To allow better control of this service call,
    // treat the highest byte of the title ID as the media type instead.
